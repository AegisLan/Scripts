...
| 修复文件 | 修改内容 |
|---|---|
| hanglvzongheng.js | 检查请求及headers类型；不区分rpid请求头大小写，仅接受字符串；缺失或无匹配时保留响应。仍使用上游的子串判定及404动作。 |
| caixinads.js | 解析JSON并遍历对象/数组，只修改精确的sday/eday/intval字段。有限数值intval完整改为0，避免多位数替换残留；沿用原2029年日期，不调整展示期限策略。 |
| 555Ad.js | 验证data/list数组；删除已识别的advert_self布局及数值type=3条目；保留空条目、未知结构及其他内容。 |
| baishitv.js | 兼容data数组/对象，仅在jumpTypeString为字符串且包含“广告”时删除条目。 |
| ithome.js | 兼容list数组/对象；仅删除已识别的flag=2或字符串“2”，保留缺失feedContent等未知条目。 |

响应脚本对非JSON、空响应、根结构变化与异常统一返回 `$done({})`，使客户端沿用原响应。无命中时也不重新序列化。百视TV/IT之家命中广告时沿用上游将集合输出为数组的形式。每个脚本只在最后调用一次 `$done`，不在catch中重试。

原作者说明保留；baishitv原文件无头部，修复版补充上游来源。修复直接应用于原文件名，不再保留 Aegis 前缀副本。只有 Configs 文件夹内的定制文件使用 Aegis 前缀。
...
以上两项已由用户明确要求忽略，不再列入待修复范围。未下载或修改。

## 验证

`node tests/script-defensive.test.cjs`
...
仅本地提交，未推送；QuantumultX.conf、AegisStartUpAds.conf等入口仍引用原脚本。这些修复尚不会自动作用于手机。批准发布后，再将实际使用的入口改到用户 Scripts 仓库中对应原文件名的 raw.githubusercontent.com 地址。文件头内原作者的示例规则是来源说明，不代表已切换到修复版。
